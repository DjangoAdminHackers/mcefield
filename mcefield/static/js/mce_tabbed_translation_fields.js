...
(function () {
    ...
    jQuery(function ($) {
        ...
        if ($('body').hasClass('change-form')) {
            ...
            //fix mcefield + translation tabs
            $('li.ui-state-default a').click(function(e){
                var target= $(this).attr('href');
                var mce_field = $(target).find('textarea').filter(':hidden');
                if (mce_field.length){
                    var id = mce_field.attr('id');
                    if(mce_field.attr('fixed_translation')){
                        return;
                    }
                    tinyMCE.execCommand("mceRemoveControl",true, id);
                    tinyMCE.execCommand("mceAddControl",true, id);
                    mce_field.attr('fixed_translation', '1')
                }
            })
        }
    });
